...
function createTree($id, $sel, $array, $currentParent, $currLevel = 0, $prevLevel = -1) {
  ...
  if ($currentParent == $category['parent_id']) {                       
    ...
    echo '<li> <label for="subfolder2">'.$moveSideString.'<a href="'.HTTPPATH.'/chess/repertorie/detail?id='.$id.'&move_id='.$category['details']['move_id'].'&parent_id='.$category['details']['move_id'].'" style="text-decoration:none;">'.$category['name'].'</a></label> <input type="checkbox" id="subfolder2" '.$checked.' />';
