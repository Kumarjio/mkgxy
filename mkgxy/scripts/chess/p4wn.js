...
var ENGINE_NAME         = 'p4wn, AKA 5k chess';
...
var FUNCTION_GO         = function (tc) {
    ...
    postMessage('info depth ' + depth + ' score cp ' + mv[2] + ' pv ' + p4_stringify_point(mv[0]) + p4_stringify_point(mv[1]) + promotion);
...
onmessage = function(e) {
    ...
    var msg = RegExp.$1;
    if (msg == 'uci') {
        postMessage('id name ' + ENGINE_NAME);
        postMessage('id author ' + ENGINE_AUTHOR);
        for (var i=0; i<ENGINE_OPTIONS.length; i++) {
            postMessage('option ' + ENGINE_OPTIONS[i]);
        }
        postMessage('uciok');
    }
    else if (msg == 'isready') postMessage('readyok');
    else if (msg == 'ucinewgame') FUNCTION_UCINEWGAME();
    else if ((/^position (?:(startpos)|fen (.*?))\s*(?:moves\s*(.*))?$/).exec(msg)) {
        FUNCTION_POSITION((RegExp.$1 == 'startpos') ? FEN_STARTPOSITION : RegExp.$2, RegExp.$3);
    }
      ...
      postMessage('unknown command ' + msg);
}
