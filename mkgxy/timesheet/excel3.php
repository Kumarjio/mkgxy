...
if($_SESSION['MM_Username']) {
	$rs = mysql_query("select * from procentris_users where user_id = '".$_GET['user_id']."'") or die("error");
	$rec = mysql_fetch_array($rs);
	$user_id = $rec['user_id'];
}
...
mysql_select_db($database_conn, $conn);
$query_rsUser = sprintf("SELECT * FROM procentris_users WHERE user_id = %s", $colname_rsUser);
...
$worksheet =& $workbook->addWorksheet();
  ...
  foreach($arr as $key => $value) { 
	...
	$worksheet->writeString($row, 0, "Clients", $formatbold);
