<?php
$pageTitle = $globalCity['name'].' Vegetable Store Configuration';
echo 'hi';exit;

$params = array();
$params['where'] = sprintf(' AND store_city_id = %s', $modelGeneral->qstr($globalCity['id']));
$details = $modelGeneral->getDetails('vegetable_store', 0, $params);
if (empty($details) || empty($details[0])) {
  ...
  $d['store_status'] = 0;
  $modelGeneral->addDetails('vegetable_store', $d);
  $details = $modelGeneral->getDetails('vegetable_store', 0, $params);
}
$details = $details[0];
if (empty($_POST)) {
                    ...
                    <div id="collapseDetail" class="panel-collapse collapse in">
                        <div class="panel-body">
                          <div class="form-group">
                            ...
                            <input name="store_name" type="text" class="inputText" id="store_name" placeholder="Enter Owner Name" value="<?php echo !empty($_POST['owner_name']) ? $_POST['owner_name'] : ''; ?>" maxlength="200" required />
                          </div>
                          <div class="form-group">
                            <strong>Owner Dwalla Email:</strong> <br />
                            <input type="text" name="owner_email_address" id="owner_email_address" class="inputText" value="<?php echo !empty($_POST['owner_email_address']) ? $_POST['owner_email_address'] : ''; ?>" placeholder="Owner Dwolla Email Address" />
                          </div>
                          <div class="form-group">
