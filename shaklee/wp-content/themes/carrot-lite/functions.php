...
function carrotlite_filter_wp_title( $title ) {
    ...
    if ( $paged >= 2 || $page >= 2 ) {
        $filtered_title .= ' | ' . sprintf( __( 'Page %s', 'blog'), max( $paged, $page ) );
    }
 ...
 */
add_filter('gallery_style', 'carrotlite_gallery_style');
function carrotlite_gallery_style() {
...
function carrotlite_customize_register($wp_customize) {
    $wp_customize->add_section('frontpage', array(
        'title' => __('Home page message', 'multipurpose')
    ));
    ...
    $wp_customize->add_setting('home_msg', array('sanitize_callback' => 'sanitize_text_field'));
    $wp_customize->add_control( new WP_Customize_Textarea_Control( $wp_customize, 'home_msg', array(
        'label' => __('Enter your home page message here:', 'multipurpose'),
        'section' => 'frontpage',
        'settings' => 'home_msg'
    ) ) );
}